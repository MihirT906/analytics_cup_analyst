...
class DashPlotlyGameRenderer:
    def __init__(self, config_file=None):
        ...
        default_config_path = os.path.join(
            ...
            "..",
            "config",
            "default_game_renderer_config.json",
        )
        try:
            ...
            print(
                f"Error loading configuration file. {default_config_path} is corrupted. Please restore it. {e}"
            )
            return

        # Load user configuration to override defaults
        try:
            default_config["display"]["figsize"] = tuple(
                default_config["display"]["figsize"]
            )
            default_config["legend"]["bbox_anchor"] = tuple(
                default_config["legend"]["bbox_anchor"]
            )
            if config_file:
        ...
        self._data_cache = (
            {}
        )  # Stores enriched_data, events_data, and frame_events per match_id
    ...
    def _get_team_color_mapping(self, enriched_data):
        ...
        if not hasattr(self, "_team_color_map"):
            all_teams = sorted(
                enriched_data["team_name"].unique()
            )  # Sort for consistency
            colors = self.config["teams"]["colors"]
            self._team_color_map = {
                team: colors[i % len(colors)] for i, team in enumerate(all_teams)
            }
    ...
    def _compute_player_masks(self, frame_data, events):
        ...
        possession_player_id = (
            events["player_possession"][0]["player_id"]
            if events["player_possession"]
            else None
        )
        passing_player_ids = set(
            event["player_id"] for event in events["passing_options"]
        )
        engagement_player_ids = set(
            event["player_id"] for event in events["on_ball_engagements"]
        )
    ...
    def plot_frame(
        self, fig, p, enriched_data, frame_events, frame_num, show_voronoi=False
    ):
        """
        ...
        frame_data, events = self._prepare_frame_data(
            enriched_data, frame_events, frame_num
        )
    ...
    def _plot_players(self, fig, frame_data, masks, enriched_data):
        ...
        for team in teams:
            ...
            regular_field_mask = (
                field_mask
                & ~possession_mask[team_mask]
                & ~passing_mask[team_mask]
                & ~engagement_mask[team_mask]
            )
            ...
            if not regular_field_players.empty:
                ...
                player_data = []
                for _, row in regular_field_players.iterrows():
                    player_data.append(
                        {
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            marker=dict(
                                ...
                                line=dict(
                                    color=self.config["players"]["styling"][
                                        "edgecolors"
                                    ],
                                    width=self.config["players"]["styling"][
                                        "edge_width"
                                    ],
                                ),
                                symbol="circle",
            ...
            if not regular_gk_players.empty:
                # Create player IDs for identification
                gk_player_data = []
                for _, row in regular_gk_players.iterrows():
                    gk_player_data.append(
                        {
                            "team_name": row["team_name"],
                            "player_id": row["player_id"],
                            "player_name": row.get("short_name", "N/A"),
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            marker=dict(
                                ...
                                opacity=self.config["players"]["styling"]["alpha"],
                                line=dict(
                                    color=self.config["players"]["events"][
                                        "passing_options"
                                    ]["edge_color"],
                                    width=self.config["players"]["events"][
                                        "passing_options"
                                    ]["edge_width"],
                                ),
                            ),
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            customdata=[
                                ...
                                for p in engagement_field_data
                            ],
                            hovertemplate="<b>Team:</b> %{customdata[0]}<br><b>Player:</b> %{customdata[1]} (ID: %{customdata[2]})<br><b>Position:</b> %{customdata[3]}<br><b>Coords:</b> (%{x:.1f}, %{y:.1f})<extra></extra>",
                            showlegend=False,
                            hoverinfo="skip",
            ...
            if self.config["players"]["events"]["possession"]["enabled"]:
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            marker=dict(
                                ...
                                size=self.config["players"]["styling"]["size"]
                                * self.config["players"]["events"]["possession"][
                                    "size_multiplier"
                                ],
                                opacity=1.0,
                                line=dict(
                            ...
                            customdata=[
                                ...
                                for p in possession_field_data
                            ],
                            hovertemplate="<b>Team:</b> %{customdata[0]}<br><b>Player:</b> %{customdata[1]} (ID: %{customdata[2]})<br><b>Position:</b> %{customdata[3]}<br><b>Coords:</b> (%{x:.1f}, %{y:.1f})<extra></extra>",
                            showlegend=False,
                            hoverinfo="skip",
                ...
                if not possession_gk_players.empty:
                    ...
                    marker_size = (
                        self.config["players"]["styling"]["size"]
                        * self.config["players"]["events"]["possession"][
                            "size_multiplier"
                        ]
                    )
    ...
    def _plot_off_ball_runs(self, fig, frame_data, events, masks):
        ...
        if events["off_ball_runs"]:
            ...
            run_events_by_player = {
                event["player_id"]: event for event in events["off_ball_runs"]
            }
                ...
                if player_id in run_events_by_player:
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            mode="lines",
                            line=dict(
                                color=self.config["players"]["events"]["off_ball_runs"][
                                    "path_color"
                                ],
                                width=2,
                                dash="10px 2px",
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            marker=dict(
                                ...
                                opacity=self.config["players"]["events"][
                                    "off_ball_runs"
                                ]["alpha"],
                                line=dict(
                                    color=self.config["players"]["styling"][
    ...
    def _add_frame_title(self, fig, frame_data, frame_num):
        ...
        timestamp = (
            frame_data["timestamp"].iloc[0]
            if "timestamp" in frame_data.columns
            else "N/A"
        )
        period = (
            frame_data["period"].iloc[0] if "period" in frame_data.columns else "N/A"
        )
        ...
        title_text = (
            f"Frame: {frame_num} | Timestamp : {time_display} | Period: {int(period)}"
        )
    ...
    def draw_voronoi(self, fig, p, frame_data, frame_num):
        ...
        team1_name = (
            frame_data[team1_mask]["team_name"].iloc[0] if team1_mask.any() else None
        )
        team2_name = (
            frame_data[team2_mask]["team_name"].iloc[0] if team2_mask.any() else None
        )
            ...
            for poly in team1:
                fig.add_trace(
                    go.Scatter(
                        ...
                        fillcolor=rgba_str1,
                        line=dict(
                            color="rgba(255, 255, 255, 0.4)", width=1, dash="dash"
                        ),
                        showlegend=False,
                        hoverinfo="skip",
                    )
    ...
    def plot_episode(
        self, match_id, start_frame, end_frame, delay=0.0, show_voronoi=False
    ):
        ...
        for frame_num in frames_to_plot:
            fig = self.plot_frame(
                fig, p, enriched_data, frame_events, frame_num, show_voronoi
            )
