...
class DashInteraction:
    def __init__(self, episode_file, config_file=None, show_voronoi=False):
        if episode_file is None:
            print(
                "Warning: No episode file provided. Please provide an episode file to load episode data."
            )
            return
    ...
    def _display_annotation_store(self):
        ...
        for shape_hash, shape_dict in self.annotation_store.items():
            box = html.Div(
                [
                    html.P(
                        [html.B("Frame Start: "), f"{shape_dict.get('frame_start')}"],
                        style={"margin": "2px 0"},
                    ),
                    html.P(
                        [html.B("Frame End: "), f"{shape_dict.get('frame_end')}"],
                        style={"margin": "2px 0"},
                    ),
                    html.P(
                        [html.B("Shape: "), f"{shape_dict['shape']['type']}"],
                        style={"margin": "2px 0"},
                    ),
                ],
                style={
    ...
    def _get_current_frame_number(self, n_intervals):
        ...
        if len(self.figures) == 0:
            return self.episode_data[
                "frame_start"
            ]  # Return START_FRAME when no figures
    ...
    def _get_episode_data(self):
        ...
        try:
            if not os.path.exists(self.episode_file):
                raise FileNotFoundError(
                    f"Episode file {self.episode_file} does not exist."
                )
        ...
        try:
            ...
            if frame_start < 0 or frame_end < 0 or frame_end < frame_start:
                raise ValueError(
                    "Frame start and end must be non-negative integers with end >= start."
                )
    ...
    def _get_episode(self):
        ...
        try:
            ...
            figures = game_renderer.plot_episode(
                self.episode_data["match_id"],
                self.episode_data["frame_start"],
                self.episode_data["frame_end"],
                delay=0,
                show_voronoi=self.show_voronoi,
            )
            print(
                f"Episode generated. Start Frame: {self.episode_data['frame_start']}, End Frame: {self.episode_data['frame_end']}"
            )
    ...
    def _create_header(self):
        ...
        return html.Div(
            ...
            children=[
                html.H1(
                    "Episode Craft Studio",
                    style={"textAlign": "center", "padding-left": "20px"},
                ),
                html.Div(
                    id="match-info",
                    children=[
                        html.P(
                            [
                                html.B("Match ID: "),
                                f"{self.episode_data['match_id']}",
                                html.Span(" | ", style={"margin": "0 10px"}),
                                html.B("Frame Range: "),
                                f"{self.episode_data['frame_start']} - {self.episode_data['frame_end']}",
                                html.Span(" | ", style={"margin": "0 10px"}),
                                html.B("Episode File: "),
                                f"{self.episode_file}",
                            ],
    ...
    def _create_plot_area(self):
        ...
        current_frame_display = html.Div(
            id="current-frame-display",
            style={
        ...
        return html.Div(
            id="plot-area",
            children=[current_frame_display, graph_display, interval],
            style={
                "display": "flex",
    ...
    def _create_annotation_area(self):
        ...
        clear_annotations_button = html.Button(
            "Clear Annotations",
        ...
        annotations_display = html.Pre(
            id="annotations-display",
            style={
                "backgroundColor": "#ffffff",
                "padding": "15px",
                "border": "1px solid black",
        ...
        return html.Div(
            id="annotation-area",
            children=[clear_annotations_button, annotations_display],
            style={"textAlign": "center"},
        )
            ...
            children=[
                ...
                html.Div(
                    children=[
                        ...
                        html.Div(
                            children=[self._create_annotation_area()],
                            style={
                                "backgroundColor": "#f0f0f0b4",
                                "border": "1px solid black",
                                "width": "20%",
    ...
    def _add_control_animation_callback(self):
        ...
        def toggle_pause_play(
            play_clicks, pause_clicks, reset_clicks, n_intervals, max_intervals
        ):

            if n_intervals >= max_intervals:
                self.is_playing = (
                    False  # If end of playback, is_playing state should be false
                )
            ...
            if button_id == "play-button":
                ...
                return (
                    ...
                    False,
                    self.episode_data["frame_start"],
                )  # Reset frame slider to start frame

            if self.is_playing:
                return (
                    True,
                    True,
                    self._get_current_frame_number(n_intervals),
                )  # Disable record and reset while playing
            else:
                return (
                    False,
                    False,
                    no_update,
                )  # Enable record and reset when paused or stopped
            ...
            if n_clicks % 2 == 1:
                ...
                if self.last_recorded_interval is not None:
                    frame_slider_marks = {
                        str(i): {"label": str(i), "style": {"color": "black"}}
                        for i in range(
                            self._get_current_frame_number(self.last_recorded_interval),
                            self.episode_data["frame_end"] + 1,
                            1,
                        )
                        if i % 10 == 0
                    }
                    new_frame_slider_marks = {
                        **frame_slider_marks,
                        str(
                            self._get_current_frame_number(self.last_recorded_interval)
                        ): {
                            "label": str(
                                self._get_current_frame_number(
                                    self.last_recorded_interval
                                )
                            ),
                            "style": {"color": "red"},
                        },
            ...
            if button_id == "play-button":
                if self.is_recording or self.last_recorded_interval is None:
                    return (
                        ...
                        no_update,
                        len_figures,
                    )  # Enable interval, keep current n_intervals
                else:
                ...
                if self.is_recording or self.last_recorded_interval is None:
                    return (
                        ...
                        no_update,
                        len_figures,
                    )  # Disable interval, keep current n_intervals
                else:
    ...
    def _add_animation_callback(self):
        ...
        @self.app.callback(
            ...
            [State("animated-figure", "figure"), State("record-button", "n_clicks")],
        )
        def update_figure(n_intervals, clear_clicks, current_figure, record_clicks):
            ...
            if self.is_recording:
                ...
                if self.last_recorded_interval is None:
                    ...
                    if n_intervals >= self.last_recorded_interval:
                        figure_index = (
                            self.last_recorded_interval - 1
                        )  # Stay on last recorded frame
                    else:
                        figure_index = n_intervals

            figure_dict = (
                self.figures[figure_index].to_dict()
                if hasattr(self.figures[figure_index], "to_dict")
                else dict(self.figures[figure_index])
            )
            ...
            if self.annotation_store:
                ...
                for shape_hash, shape_dict in self.annotation_store.items():
                    ...
                    current_frame = self._get_current_frame_number(n_intervals or 0)
                    if frame_start < current_frame and (
                        frame_end is None or frame_end > current_frame
                    ):  # Ongoing shape
        ...
        def capture_annotations(relayout_data, clear_clicks, n_intervals):
            if self.is_recording:
                ...
                if relayout_data and "shapes" in relayout_data:
                    ...
                    current_shapes_hashes = [
                        self._get_shape_hash(shape) for shape in current_shapes
                    ]
                    ...
                    for shape in current_shapes:
                        ...
                        if shape_hash not in self.annotation_store:
                            self.annotation_store[shape_hash] = {
                    ...
                    for shape_hash, shape_dict in list(self.annotation_store.items()):
                        # print(shape_dict)
                        if (shape_hash in removed_shape_hashes) and (
                            shape_dict["frame_start"] <= current_frame
                        ):
